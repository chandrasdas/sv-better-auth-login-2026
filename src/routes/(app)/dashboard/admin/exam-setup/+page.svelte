...
<main class="mx-auto min-h-[101vh] max-w-7xl px-4 py-12 sm:px-6 lg:px-8" in:fade={{ duration: 400 }}>
	...
	<div class="mb-10 sm:flex sm:items-start sm:justify-between relative overflow-hidden rounded-3xl border border-white/5 bg-linear-to-b from-white/5 to-transparent p-8 shadow-2xl">
		...
		<div class="relative z-10 flex-1 mb-6 sm:mb-0">
			<h1 class="text-4xl font-bold tracking-tight text-white">Exam Setup</h1>
			<p class="mt-3 max-w-2xl text-lg text-slate-400">Configure full marks for each subject. Check 'Include in Marksheet' to add a subject to the exam.</p>
		</div>
		...
		<div class="relative z-10 w-full sm:w-auto mt-6 sm:mt-0">
			<div class="flex flex-col items-end gap-4">
				...
				<div class="flex flex-wrap justify-end gap-3 w-full sm:w-auto">
					<select bind:value={currentSession} onchange={fetchSetups} class="rounded-xl border border-white/10 bg-white/5 py-2.5 pl-3 pr-8 text-sm text-white focus:border-indigo-500 focus:bg-white/10 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition">
				...
				<div class="overflow-hidden rounded-3xl border border-white/5 bg-white/2 shadow-2xl ring-1 ring-white/5">
					<table class="min-w-full divide-y divide-white/5">
						<thead class="bg-white/5">
							<tr>
								<th scope="col" class="py-2 pl-4 pr-3 text-left text-sm font-semibold text-white sm:pl-6 w-16">SL</th>
								<th scope="col" class="px-3 py-2 text-left text-sm font-semibold text-white">Subject Name</th>
								<th scope="col" class="px-3 py-2 text-center text-sm font-semibold text-white w-32">Include in Marksheet</th>
								<th scope="col" class="px-3 py-2 text-center text-sm font-semibold text-white w-32">Include in Total</th>
								<th scope="col" class="px-3 py-2 text-left text-sm font-semibold text-white w-32">Sort Order</th>
								<th scope="col" class="px-3 py-2 text-left text-sm font-semibold text-white w-48">Full Marks</th>
								<th scope="col" class="px-3 py-2 text-left text-sm font-semibold text-white w-48">Pass Marks</th>
							</tr>
						</thead>
						<tbody class="divide-y divide-white/5 bg-transparent">
							{#each displaySubjects as subject, i (subject.id)}
							<tr class="transition hover:bg-white/10 even:bg-white/2">
								<td class="whitespace-nowrap py-2 pl-4 pr-3 text-sm font-medium text-slate-300 sm:pl-6">
									{i + 1}
								</td>
								<td class="whitespace-nowrap px-3 py-2 text-sm text-slate-200 font-medium">
									{subject.name}
								</td>
								<td class="whitespace-nowrap px-3 py-2 text-sm text-slate-400 text-center">
									<input 
										type="checkbox"
										bind:checked={includeMarksheetInputs[subject.id]}
										class="h-5 w-5 rounded border-white/10 bg-white/5 text-indigo-500 focus:ring-indigo-500 focus:ring-offset-slate-900 transition mx-auto"
									>
								</td>
								<td class="whitespace-nowrap px-3 py-2 text-sm text-slate-400 text-center">
									<input 
										...
										class="h-5 w-5 rounded border-white/10 bg-white/5 text-indigo-500 focus:ring-indigo-500 focus:ring-offset-slate-900 transition mx-auto disabled:opacity-50 disabled:cursor-not-allowed"
									>
								</td>
								<td class="whitespace-nowrap px-3 py-2 text-sm text-slate-400">
									<input 
										type="number"
										min="0"
										...
										placeholder="0"
										disabled={!includeMarksheetInputs[subject.id]}
										class="block w-full rounded-xl border border-white/10 bg-white/5 py-1.5 px-3 text-sm text-white placeholder-slate-500 focus:border-indigo-500 focus:bg-white/10 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition disabled:opacity-50 disabled:cursor-not-allowed"
									>
								</td>
								<td class="whitespace-nowrap px-3 py-2 text-sm text-slate-400">
									<input 
	...
	<div class="mt-8 flex items-center justify-end gap-4">
		{#if saveMessage}
			<span class="text-sm font-medium {saveMessage.includes('Failed') ? 'text-rose-400' : 'text-emerald-400'} transition-opacity duration-300" in:fade>
				{saveMessage}
			</span>
		{/if}
		<button 
			onclick={handleSave} 
			disabled={isSaving}
			class="rounded-xl bg-indigo-500 px-6 py-2.5 text-sm font-medium text-white shadow-xs hover:bg-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-slate-900 disabled:opacity-50 disabled:cursor-not-allowed transition"
		>
