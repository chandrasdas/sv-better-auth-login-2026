...
<div class="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-sans flex selection:bg-indigo-500/30">
	<!-- Sidebar -->
	<aside class="w-64 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex-col hidden md:flex sticky top-0 h-screen shadow-sm transition-colors">
		<div class="h-16 flex items-center px-6 border-b border-slate-100 dark:border-slate-800 transition-colors">
			<a href={resolve('/')} class="flex items-center gap-3 group transition hover:opacity-80">
				...
				<span class="text-lg font-bold tracking-tight text-slate-800 dark:text-slate-100 transition-colors">{APP_NAME}</span>
			</a>
		</div>
				...
				{#each links as link (link.name)}
					<li>
						<a
							...
							class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white"
						>
		...
		<div class="p-4 border-t border-slate-100 dark:border-slate-800 transition-colors">
			<div class="mb-4 px-2">
				<div class="text-sm font-medium text-slate-800 dark:text-slate-200 transition-colors">{data.user?.name}</div>
				{#if data.user?.role}
					<div class="mt-1 inline-flex items-center rounded-md bg-indigo-50 dark:bg-indigo-500/10 px-2 py-0.5 text-xs font-medium text-indigo-700 dark:text-indigo-400 ring-1 ring-inset ring-indigo-700/10 dark:ring-indigo-400/20 uppercase tracking-wide">
						{data.user.role}
					</div>
				{/if}
			</div>
			...
			<button
				...
				disabled={loading}
				class="w-full flex items-center justify-center gap-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 shadow-sm transition hover:bg-slate-50 dark:hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 disabled:opacity-50"
			>
				{#if loading}
	...
	<div class="flex-1 flex flex-col min-h-0 overflow-hidden">
		<!-- Mobile Header -->
		<header class="md:hidden h-16 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center justify-between px-4 shrink-0 transition-colors">
			<a href={resolve('/')} class="flex items-center gap-2">
				...
				<span class="font-bold text-slate-800 dark:text-slate-100 transition-colors">{APP_NAME}</span>
			</a>
			<!-- We'll keep mobile nav simple or omitted for now, user can use desktop view -->
		</header>

		<!-- Main Content Area -->
		<main class="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-950 p-6 md:p-8 transition-colors">
			{@render children()}
		</main>
	</div>
</div>
